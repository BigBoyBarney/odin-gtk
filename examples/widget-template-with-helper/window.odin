...
startup :: proc "c" (app: ^gtk.Application, _: rawptr) {
    ...
    template_children := []helper.Template_Child {
        {id = "my-button", field_name = "button"},
    }
	...
	*/
    my_box_class_init :: proc "c" (
        class: ^gobj.TypeClass,
        data: glib.pointer,
    ) {
        ...
        gtk.widget_class_bind_template_callback_full(
            widget_class,
            "my_button_clicked",
            clicked_callback,
        )
    ...
    my_button_clicked :: proc "c" (button: ^gtk.Button, _: rawptr) {
        ...
        button_clicked_cstring := fmt.caprintf(
            "Clicked %d times!",
            my_box.button_clicked,
        )
